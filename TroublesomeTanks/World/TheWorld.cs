...
namespace TroublesomeTanks.World
{
    ...
    public class TheWorld
    {
        ...
        public int LiveBulletCount()
        {
            ...
            {
                numBullets += m_Tanks[tankIndex].GetBulletList().Count;
            }
            return numBullets;
        }
                ...
                for (int i = bulletList.Count - 1; i >= 0; --i)
                {
                    ...
                    for (int tankIndex2 = 0; tankIndex2 < m_Tanks.Count; tankIndex2++)
                    {
                        ...
                        if (bulletList[i].Collide(m_Tanks[tankIndex2], out collisionNormal))
                        {
                            ...
                            Microsoft.Xna.Framework.Audio.SoundEffectInstance tankClangSound = TroublesomeTanks.Instance().GetSoundManager().GetSoundEffectInstance(tankClang);
                            tankClangSound.Play();
                            collided = true;
                            break;
                        }
